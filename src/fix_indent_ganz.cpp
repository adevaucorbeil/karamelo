...
FixIndentGanz::FixIndentGanz(MPM *mpm, vector<string> args)
    : Fix(mpm, args) {
  if (args.size() < Nargs) {
  ...
  if (group->pon[igroup].compare("particles") != 0 &&
    ...
    error->all(FLERR, "fix_indent_ganz needs to be given a group of nodes" +
                          group->pon[igroup] + ", " + args[2] +
                          " is a group of " + group->pon[igroup] + ".\n");
  }
  cout << "Creating new fix FixIndentGanz with ID: " << args[0] << endl;
  id = args[0];
...
FixIndentGanz::~FixIndentGanz() {}

void FixIndentGanz::init() {}

void FixIndentGanz::setup() {}

void FixIndentGanz::setmask() {
  mask = 0;
  mask |= INITIAL_INTEGRATE;
}

void FixIndentGanz::initial_integrate() {
  // cout << "In FixIndentGanz::initial_integrate()\n";
